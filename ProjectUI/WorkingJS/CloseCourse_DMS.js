...
app.service('CloseCourse_DMSService', function ($http, $location) {
    ...
    this.GetFacultyList = function (Agency_Id) {
        return $http.get(this.AppUrl + '/Automation/GetFacultyList?Agency_Id=' + Agency_Id);
    };
    ...
    $scope.GetReport = function (ReportInput) {
        ...
        MarksReportAsPerDMSService.GetMarksReport(ReportInput).then(function success(data) {
            ...
            angular.forEach($scope.MarksReport, function (value, key) {
                ...
                if (value.IsPresentInPreTest == 0) { value.PreTest_MarksObtained = '0' }
                if (value.IsPresentInPostTest == 0) { value.PostTest_MarksObtained = '0' }
            });
            $scope.data = {};
